...
> **Status:** early implementation. The server validates boot configuration,
> discovers and validates source candidates, records managed output in SQLite,
> atomically publishes a symlink library, and reconciles source changes through
> bounded periodic scans. The CLI and shared-contract workspaces are placeholders.
> See the [server development guide](server/README.md) and the [Gonic
> integration-test guide](server/GONIC.md) for real Subsonic-client testing.

## Workspaces

| Workspace | Purpose |
| --- | --- |
| [`server/`](server/README.md) | Linux daemon: scan, validate, publish, state, and source watching. |
| [`cli/`](cli/README.md) | Reserved workspace for a future command-line client. |
| [`packages/contracts/`](packages/contracts/README.md) | Reserved workspace for shared contracts. |

## Core invariants

- Source trees are immutable: Siftone never edits tags, moves, renames, deletes, chmods, or writes beside originals.
- Only the server process may mutate generated-library, staging, state, and backup roots; those roots must not overlap source roots.
