...
`bash`, `curl`, and `sqlite3`. Install Gonic separately; it is not a Bun
dependency. On startup, the helper checks the listener with `ss` when available,
then `lsof`, and finally Bash's built-in TCP connection support.
...
`verify` checks ping, requires at least one indexed SQLite track, and performs
a 4 KiB ranged Subsonic stream request.
...
2. Start Siftone. Its boot publication pass validates every source candidate,
   ...
   directories in `.siftone-staging`, and atomically publishes each new album
   symlink directory into `/home/damian/Music`.
...
3. Verify Gonic picked up the generated library:
   ...
   ```bash
   find /home/damian/Music -type l | wc -l
   server/scripts/gonic-test.sh tracks
   server/scripts/gonic-test.sh verify
   ```
...
```bash
find /home/damian/Music -type l | wc -l
find -L /home/damian/Music -type l | wc -l # broken links
server/scripts/gonic-test.sh tracks
server/scripts/gonic-test.sh verify
```
...
- Each album directory rename is atomic on the generated-library filesystem;
  an entire multi-album boot pass is **not** batch-atomic. If a later album
