...
export const SCHEMA_SQL = `
	...
	CREATE TABLE album_versions (
		...
		CHECK ((state IN ('pending', 'current') AND retired_at_ns IS NULL) OR (state = 'retired' AND retired_at_ns IS NOT NULL))
	) STRICT;
	...
	CREATE TABLE imports (
		...
		destination_path TEXT COLLATE BINARY UNIQUE CHECK (destination_path IS NULL OR ${absolutePathCheck("destination_path")}),
		current_version_id TEXT UNIQUE REFERENCES album_versions(id),
		CHECK ((destination_path IS NULL) = (current_version_id IS NULL)),
	...
	CREATE INDEX source_files_import_idx ON source_files(import_id);
